...
public class CucinaGiapponeseDaoJDBC implements CucinaGiapponeseDAO {

    private final Connection connection;



    public CucinaGiapponeseDaoJDBC(Connection connection) {
        this.connection = connection;
    }

    @Override
    public String getNomeById(int id) {
        String query = "SELECT nome FROM cucina_giapponese WHERE id = ?";
    ...
    public String getIngredientiById(int id) {
        String query = "SELECT ingredienti FROM cucina_giapponese WHERE id = ?";
        try (PreparedStatement ps = connection.prepareStatement(query)) {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
    ...
    public String getImmagineById(int id) {
        String query = "SELECT immagine FROM cucina_giapponese WHERE id = ?";
        try (PreparedStatement ps = connection.prepareStatement(query)) {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
    ...
    public double getPrezzoById(int id) {
        String query = "SELECT prezzo FROM cucina_giapponese WHERE id = ?";
        try (PreparedStatement ps = connection.prepareStatement(query)) {
            ps.setInt(1, id);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
